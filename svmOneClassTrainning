...
int main()
{
    ...
    svm_->setType(cv::ml::SVM::ONE_CLASS);
    ...
    if(!posSample.empty()){
        cv::Ptr<cv::ml::TrainData> trainData =
              cv::ml::TrainData::create(posSample, cv::ml::SampleTypes::ROW_SAMPLE,
                                        labels);
        ...
        ofstream outList(string(DATAPATH + "neg.lst"),ofstream::out);
        outList.close();
        outList.open(string(DATAPATH + "neg.lst"),ofstream::app);
        if(!outList.is_open()){
        ...
        while(getline(negList,negLine)){
            ...
            if(!neg.empty()){
                ...
                hog.detectMultiScale(neg,foundLocations,0.07,Size(4,4),Size(0,0),1.05,2,false);
